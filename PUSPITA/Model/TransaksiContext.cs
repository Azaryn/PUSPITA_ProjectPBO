...
namespace PUSPITA.Model
{
    class TransaksiContext
    {
        ...
        public int TambahTransaksi(string nama, DateTime date, int idpetani, string metode, string pengiriman)
        {
            using (NpgsqlConnection kon = new NpgsqlConnection(KoneksiString))
            {
                kon.Open();
                string query = "INSERT INTO transaksi (nama, tanggal, id_petani, metode_pembayaran, metode_pengiriman) " +
               "VALUES (@nama, @tanggal, @id_petani, @metode_pembayaran, @metode_pengiriman) " +
        ...
        public bool TambahDetail(int id_transaksi, int id_produk, int jumlah, decimal harga)
        {
            ...
            {
                kon.Open();
                string query = "INSERT INTO detail_transaksi (id_transaksi, id_produk, jumlah, harga) VALUES (@id_transaksi, @id_produk, @jumlah, @harga)";
                using (NpgsqlCommand cmd = new NpgsqlCommand(query, kon))
                {
